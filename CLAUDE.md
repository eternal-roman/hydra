...
- **Hard risk caps (PR-B)** — `max_position_pct` applies **after** brain `size_multiplier` and caps gross inventory (notional/equity). Peak equity never rebases downward on balance seed/resume. Exception: a live first-seed of an unfunded quote pool must not treat the constructor `--balance` split as a real high-water mark — that printed DD 100% and armed the 15% BUY halt on accounts that simply hold no USD cash. A snapshot-restored peak (above the dummy split) is still preserved. Portfolio max DD ≥ 15% sticky-blocks new BUYs (SELL still allowed).
