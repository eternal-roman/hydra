...
| `HYDRA_WS_HOST` | dashboard | Bind address for the dashboard WebSocket. Default **`127.0.0.1`** (v2.32.1). Inbound commands need the per-process token, but the OUTBOUND path sends `latest_state` — full balances, positions, journal — to any accepted socket, and a missing `Origin` header is allowed (non-browser clients). Binding `0.0.0.0` therefore served the whole account with no credential. Set `=0.0.0.0` only behind a proxy that terminates auth. |
